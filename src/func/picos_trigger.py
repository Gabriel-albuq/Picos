...
def trigger_test(frame, perc_top, perc_bottom):
    ...
    cropped_image_top = frame.copy()[
        y1:y2, x1:x2
    ]   # Cortando a região de interesse (ROI)
    ...
    x1, y1 = (
        int((frame_width / 2) - 150),
        line_limit_bottom - 20,
    )  # Posição do canto superior esquerdo

    cropped_image_bottom = frame.copy()[
        y1:y2, x1:x2
    ]   # Cortando a região de interesse (ROI)
    mean_color_bottom = cv2.mean(cropped_image_bottom)[
        :3
    ]   # Apenas os canais BGR

    # Definir os valores aproximados para preto/branco e marrom
    color_black_white = np.array(
        [128, 128, 128]
    )  # Cinza médio para preto/branco
    ...
    def closest_color(mean_color, color1, color2):
        dist1 = np.linalg.norm(
            np.array(mean_color) - color1
        )  # Distância da cor 1

        dist2 = np.linalg.norm(
            np.array(mean_color) - color2
        )  # Distância da cor 2
    ...
    return trigger_result, trigger_top_value, trigger_bottom_value
...
def trigger_frame(frame, trigger_top_value, trigger_bottom_value, perc_top, perc_bottom):
    ...
    if trigger_top_value == 1:
        cv2.line(
            frame,
            (0, line_limit_top),
            (frame_width, line_limit_top),
            (0, 255, 0),
            2,
        )
        cv2.rectangle(
            frame,
            (0, line_limit_top - rectangle_height // 2),
            (rectangle_width, line_limit_top + rectangle_height // 2),
            (0, 255, 0),
            -1,
        )
        cv2.putText(
    ...
    if trigger_bottom_value == 1:
        cv2.line(
            frame,
            (0, line_limit_bottom),
            (frame_width, line_limit_bottom),
            (0, 255, 0),
            2,
        )
        cv2.rectangle(
            frame,
            (0, line_limit_bottom - rectangle_height // 2),
            (rectangle_width, line_limit_bottom + rectangle_height // 2),
            (0, 255, 0),
            -1,
        )
        cv2.putText(
